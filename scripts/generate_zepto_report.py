...
demand_lookup = dict(zip(zip(sop['MH_clean'], sop['DH_clean']), sop['w1']))
restriction_lookup = dict(zip(zip(sop['MH_clean'], sop['DH_clean']), sop['Vehicle restrictions']))

def get_node_demand(mh_name, dh_name):
    clean_name = dh_name.split('_part')[0]
    full_dem = demand_lookup.get((mh_name, clean_name), 0)
    
    if '_part' not in dh_name:
        return full_dem
        
    res_str = str(restriction_lookup.get((mh_name, clean_name), '')).strip()
...
for _, row in routes.iterrows():
    ...
    # Filter out the MH from the stops to get just the DHs
    ...
    for i, dh in enumerate(dhs):
        ...
        dem = get_node_demand(mh_name, dh)
        restr = str(restriction_lookup.get((mh_name, clean_name), 'None')).strip()
...
# Standardize columns
cols = ['Material Hub', 'Route Type', 'Assigned Vehicle', 'Vehicle Capacity (kg)', 'Total Route Demand (kg)', 'Utilization (%)', 'Total Distance (km)']
